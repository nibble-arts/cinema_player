...
# bufixes
# Thomas Winkler <thomas.winkler@iggmp.net>

#   added kill_process to end mplayer process
#   listen to ANS_ in stdout for correct answers from mplayer
#       instead of split("=") != 2 (leads to messy answers with strings containing =>)
...
class Properties(object):
    ...
    def _getter(self, item):
        ...
        '''
        command_string = 'pausing_keep_force get_property %s' % item['command']
    ...
    def _setter(self, args, item):
        ...
        # Sending the command
        command_string = 'pausing_keep_force set_property %s %s' %\
    ...
    def __new__(cls, *args, **kwargs):
        ...
        for item in property_dict.keys():
            ...
            if property_dict[item]['set'] is True:
                ...
                prop = property(fget=partial(cls._getter, item=property_dict[item]),
                                fset=partial(cls._setter, item=property_dict[item]),
                                doc=doc)
...
class Player(object):
    ...
    def _new_get_method(self):
        ...
        command_string = 'pausing_keep_force %s' % item['command']

        self._send_command(command_string)
        answer = ''

        # Skipping a line that is not an answer(and other garbage)
        while (len(answer.split('=')) != 2 and not 'ANL_' in answer) :
            answer = self._player_answer.readline()
    ...
    def _new_args_method(self, *args):
        ...
        if count_args <= count_args_types:
            ...
            if self._debug:
                print 'EXECUTED ARGS COMMAND:', command_string

            # Sending the command
            self._send_command(command_string)
        else:
            # self._send_command('quitcreate_new_process')
    ...
    def kill_process(self):
        if self._debug:
            print "terminate process "+str(self._pid)
            pass
        self._process.kill()
    ...
    def connect_to_process(self):
        ...
        if (self._pid is not None) and (pid_exists(self._pid)):
            ...
            if self._debug:
                print 'PROCESS DOES NOT EXISTS; CREATE NEW PROCESS'
