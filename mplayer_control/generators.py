...
class CmdDictGenerator(object):
    ...
    def __init__(self, mplayer='mplayer'):
        # start process to get command list
        self._process = Popen([mplayer, '-input', 'cmdlist'], stdout=PIPE)

        self._cmdlist = [line for line in self._process.stdout][:-1]
        self._doc_file = open(
                       os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                       'data/mplayer_cmdlist_docs'), 'r').readlines()
        self._cmdlist_dict = {}

        # kill command list process
        self._process.kill()
